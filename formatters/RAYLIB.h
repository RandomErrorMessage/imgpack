static int imgpack_formatter_RAYLIB(struct ImgPackContext *ctx, FILE *f) {
	...
	fprintf(f, "} __attribute__ ((__packed__)) %s_Id;\n\n", name);
	...
	fprintf(f, "%s_DEF void %s_DrawNP(%s_Id id, float x, float y, Color color, int anchor);\n", name, name, name);
	fprintf(f, "%s_DEF int %s_Draw(%s_Id id, float x, float y, Color color, int anchor, const Vector2 *point);\n", name, name, name);
	fprintf(f, "%s_DEF int %s_DrawEx(%s_Id id, float x, float y, float rotation, float scale, Color color, int anchor, const Vector2 *point);\n", name, name, name);
	fprintf(f, "%s_DEF void %s_DrawExNP(%s_Id id, float x, float y, float rotation, float scale, Color color, int anchor);\n", name, name, name);
	fprintf(f, "%s_DEF Texture %s_GetTexture(void);\n", name, name);
	fprintf(f, "%s_DEF const Vector2 %s_GetScale(void);\n", name, name);
	fprintf(f, "%s_DEF const Rectangle %s_GetFrame(%s_Id id);\n", name, name, name);
	fprintf(f, "%s_DEF const Vector2 %s_GetOffset(%s_Id id);\n", name, name, name);
	fprintf(f, "%s_DEF const Vector2 %s_GetSourceSize(%s_Id id);\n", name, name, name);
	fprintf(f, "%s_DEF const Vector2 %s_GetOrigin(%s_Id id);\n", name, name, name);
	fprintf(f, "\n");
	fprintf(f, "#endif\n\n");
	...
	fprintf(f, "void %s_DrawNP(%s_Id id, float x, float y, Color color, int anchor) {\n", name, name);
	fprintf(f, "  if (id) {\n");
	...
	fprintf(f, "int %s_Draw(%s_Id id, float x, float y, Color color, int anchor, const Vector2 *point) {\n", name, name);
	fprintf(f, "  if (id) {\n");
	fprintf(f, "    x += (anchor & 1 ? 0 : anchor & 2 ? -%s_SourceSize[id].x : -%s_Origin[id].x - %s_Offset[id].x);\n", name, name, name);
	fprintf(f, "    y += (anchor & 4 ? 0 : anchor & 8 ? -%s_SourceSize[id].y : -%s_Origin[id].y - %s_Offset[id].y);\n", name, name, name);
	fprintf(f, "    Rectangle destRec = {x + %s_Offset[id].x, y + %s_Offset[id].y, %s_Frame[id].width, %s_Frame[id].height};\n", name, name, name, name);
	...
	fprintf(f, "int %s_DrawEx(%s_Id id, float x, float y, float rotation, float scale, Color color, int anchor, const Vector2 *point) {\n", name, name);
	fprintf(f, "  if (id) {\n");
	...
	fprintf(f, "void %s_DrawExNP(%s_Id id, float x, float y, float rotation, float scale, Color color, int anchor) {\n", name, name);
	fprintf(f, "  if (id) {\n");
	fprintf(f, "    Rectangle sourceRec = %s_Frame[id];\n", name);
	fprintf(f, "    Vector2 origin = %s_Origin[id];\n", name);
	fprintf(f, "    if (anchor & 1) origin.x = 0; else if (anchor & 2) origin.x = %s_SourceSize[id].x;\n", name);
	...
	fprintf(f, "const Rectangle %s_GetFrame(%s_Id id) {\n  return %s_Frame[id];\n}\n\n", name, name, name);
	fprintf(f, "const Vector2 %s_GetOffset(%s_Id id) {\n  return %s_Offset[id];\n}\n\n", name, name, name);
	fprintf(f, "const Vector2 %s_GetSourceSize(%s_Id id) {\n  return %s_SourceSize[id];\n}\n\n", name, name, name);
	fprintf(f, "const Vector2 %s_GetOrigin(%s_Id id) {\n  return %s_Origin[id];\n}\n\n", name, name, name);
